...
impl From<BufferOverflow> for RenderError {
...
impl<'buf> OpenDeckRenderer<'buf> {
    ...
    ) -> Result<Option<Sysex7<&'buf mut [u8]>>, RenderError> {
        let mut buf = Vec::new();
...
impl ValueSize {
    fn push(&self, value: u16, mut buf: Buffer) -> Result<Buffer, RenderError> {
        match self {
            ValueSize::OneByte => {
                if value >= 128 {
...
impl crate::config::FirmwareVersion {
    fn push(self, mut buf: Buffer, value_size: &ValueSize) -> Result<Buffer, RenderError> {
...
mod tests {
    ...
    fn assert_sysex(act: Result<Option<Sysex7<&mut [u8]>>, RenderError>, exp: &[u8]) {
    ...
    fn should_render_u16() {
        let buf = Vec::new();
        assert_eq!(ValueSize::TwoBytes.push(10000, buf).unwrap(), &[0x4E, 0x10]);
    }
    ...
    #[test]
    fn should_accept_value_lt_128_in_one_byte_mode() {
        let buf = Vec::new();
        assert_eq!(ValueSize::OneByte.push(127, buf).unwrap(), &[0x7F]);
    }
    ...
    fn should_render_serial_number() {
        ...
        assert_sysex(
            ...
            &[
                0xF0, 0x00, 0x53, 0x43, 0x01, 0x00, 0x53, 0x00, 0x11, 0x00, 0x22, 0x00, 0x33, 0xF7,
            ],
        );
    }
    ...
    fn should_render_global_osc_section() {
        ...
        assert_sysex(
            ...
            &[
                0xF0, 0x00, 0x53, 0x43, 0x01, 0x00, 0x00, 0x00, 0x00, 0x03, 0x00, 0x04, 0x00, 0x00,
                0x46, 0x28, 0xF7,
            ],
        );
    }
